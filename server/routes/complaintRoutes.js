...
router.post("/create", authenticateUser, isCitizen, async (req, res) => {
  try {
    const { title, description, category, wardNumber, location, imageUrl } =
      req.body;
    ...
    if (!title || !description || !category || !wardNumber) {
      return res
        .status(400)
        .json({
          message: "Title, description, category, and ward number are required",
        });
    }
  ...
  async (req, res) => {
    try {
      ...
      const complaints = await Complaint.find({
        wardNumber: councillor.wardNumber,
      })
...
router.get("/:id", authenticateUser, async (req, res) => {
  try {
    ...
    const isAssignedCouncillor = complaint.assignedCouncillor?._id.equals(
      req.user.userId,
    );

    if (
      !isCitizenOwner &&
      !isAssignedCouncillor &&
    ...
    res
      .status(500)
      .json({ message: "Failed to fetch complaint", error: error.message });
  }
});
  ...
  async (req, res) => {
    try {
      ...
      if (complaint.wardNumber !== councillor.wardNumber) {
        return res
          .status(403)
          .json({ message: "You can only update complaints in your ward" });
      }
  ...
  async (req, res) => {
    try {
      ...
      complaints.forEach((complaint) => {
        stats.categoryBreakdown[complaint.category] =
          (stats.categoryBreakdown[complaint.category] || 0) + 1;
      });
